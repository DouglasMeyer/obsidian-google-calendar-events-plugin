...
const refreshAccessToken = async (
	...
	const refreshBody = {
		...
		client_id: useCustomClient
			? plugin.settings.googleClientId?.trim()
			: "PUBLIC_CLIENT_ID",
		client_secret: useCustomClient
	...
	const { json: tokenData } = await requestUrl({
		...
		url: useCustomClient
			...
			: `${plugin.settings.googleOAuthServer}/api/google/refresh`,
...
// const exchangeCodeForTokenDefault = async (plugin: GoogleEventsPlugin, state:string, verifier:string, code: string): Promise<boolean> => {
...
// 	const CLIENT_ID = useCustomClient
...
// 	state: string
// ): Promise<void> {
// 	const plugin = GoogleEventsPlugin.getInstance();
...
// 		GoogleEventsPlugin.getInstance(),
// 		state,
	...
	authSession.server = http
		.createServer(async (req: IncomingMessage, res: ServerResponse) => {
			...
			try {
				...
				// } else {
				// 	token = await exchangeCodeForTokenDefault(
				// 		plugin,
				// 		authSession.state,
				// 		authSession.verifier,
				// 		code
