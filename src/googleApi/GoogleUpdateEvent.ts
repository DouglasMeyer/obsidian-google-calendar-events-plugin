...
export async function googleUpdateEvent(
	...
	if (updateAllOccurrences && event.recurringEventId) {
		const recurringEvent = await getEvent(
		...
		if (event.start.dateTime) {
			event.start.dateTime = window
				.moment(event.start.dateTime)
				.date(window.moment(recurringEvent?.start.dateTime).date())
				.format();
			event.end.dateTime = window
				.moment(event.end.dateTime)
				.date(window.moment(recurringEvent?.end.dateTime).date())
				.format();
		} else {
			event.start.date = window
				.moment(recurringEvent?.start.date)
				.format("YYYY-MM-DD");
			event.end.date = window
				.moment(recurringEvent?.end.date)
				.format("YYYY-MM-DD");
		}
	...
	const updatedEvent = await callRequest(
		`https://www.googleapis.com/calendar/v3/calendars/${calenderId}/events/${event.id}`,
		"PUT",
		event
	);
	const calendars = await googleListCalendars();
	updatedEvent.parent = calendars.find(
		(calendar) => calendar.id === calenderId
	);
...
export async function updateEvent(
	...
	try {
		...
		switch (error.status) {
			...
			default:
				createNotice(
					`Google Event ${event.summary} could not be updated.`
				);
