...
export async function googleSwitchCalendar(
	...
	if (calenderId === "") {
		throw new GoogleApiError(
			"Could not switch Calendar for Event because no default calendar selected in Settings",
			null,
			999,
	...
	await callRequest(
		...
		}/move?destination=${newCalendarId}`,
		"POST",
		event
	);
...
export async function switchCalendar(
	...
	try {
		const updatedEvent = await googleSwitchCalendar(event, newCalendarId);
		...
		switch (error.status) {
			...
			default:
				createNotice(
					`Google Event ${event.summary} could not switch calendar.`
				);
